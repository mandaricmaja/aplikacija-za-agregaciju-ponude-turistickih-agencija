...
<div>
  <br><br>
    ...
    <div class="title ">
      <h1>IZMIJENI TURISTIČKU PONUDU</h1>
    </div>
     ...
     <div class="menu-all-place">
      <div class="menu-full">
        <div class="menu-container">
          <div class="row">
            <form @submit.prevent="updateTravelOffer" role="form" class="php-email-form">
              <div class="row">
              ...
              <button class="btn btn-primary mt-4">Izmijeni ponudu</button>
            </form>
          </div>
        </div>
      </div>
...
export default { 
  ...
  methods: {
    async updateTravelOffer() {
      try {
        const args = {
          method: "PUT",   
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({naziv_ponude: this.naziv_ponude, lokacija: this.lokacija, mjesto_polaska: this.mjesto_polaska, naziv_turisticke_agencije: this.naziv_turisticke_agencije, datum_putovanja: this.datum_putovanja, broj_dana: this.broj_dana, cijena: this.cijena, vrsta_prijevoza: this.vrsta_prijevoza, kategorija_putovanja: this.kategorija_putovanja, opis_ponude: this.opis_ponude, fotografija: this.fotografija}) 
        }
        const response = await fetch(`http://localhost:8000/offers/${this.id}`, args)
  ...
  async beforeRouteEnter(to, from, next) {
    if(to.params.id != undefined) {
      try {
        ...
        let response = await fetch(`http://localhost:8000/offers/${to.params.id}`, args)
        let data = await response.json()
        next(vm=>(vm.naziv_ponude=data.naziv_ponude, vm.lokacija=data.lokacija, vm.naziv_turisticke_agencije=data.naziv_turisticke_agencije, vm.mjesto_polaska=data.mjesto_polaska, vm.datum_putovanja=data.datum_putovanja, vm.broj_dana=data.broj_dana, vm.cijena=data.cijena, vm.vrsta_prijevoza=data.vrsta_prijevoza, vm.kategorija_putovanja=data.kategorija_putovanja, vm.opis_ponude=data.opis_ponude, vm.fotografija=data.fotografija))
      } catch (error) {
          console.error(error)
        }
    }
