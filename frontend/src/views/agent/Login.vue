<template>
    ...
    <h1>PRIJAVA TURISTIČKOG AGENTA</h1>
    <br>
        ...
        <div class="menu-container">
          <div class="row">
            <form @submit.prevent="loginAgent">
              ...
              />
              <br/>
              <button class="btn btn-primary mt-4">Prijavi se</button>
            </form>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async loginAgent() {
      try {
        let response = await axios.post('http://localhost:8000/agents/login', {
          email: this.email,
        ...
        if(response.status === 200) {
          ...
          this.poruka = 'Neispravna e-mail adresa ili lozinka!';
          console.log(error);
        }
    }
  }
